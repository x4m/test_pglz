...
static uint16 rolling_hash[PGLZ_MAX_HISTORY_LISTS];
 ...
 * ----------
 */
#define pglz_hist_add(_hs,_he,_hn,_recycle,_s,_e, _mask, _hash)	\
do {									\
			...
			*__myhsp = _hn;													\
			...
			uint __rhindex = ((uint)_s) % PGLZ_HISTORY_SIZE;				\
			uint __rhnext = (__rhindex + 1) % PGLZ_HISTORY_SIZE;			\
			_hash[__rhnext] = _hash[__rhindex] + *_s;						\
} while (0)
...
pglz_find_match(int16 *hstart, const char *input, const char *end,
				int *lenp, int *offp, int good_match, int good_drop, int mask, uint16 *rhash)
{
	PGLZ_HistEntry *hent;
	int16		hentno;
	int32		len = 0;
			  ...
			  const PGLZ_Strategy *strategy)
{
	...
	memset(rolling_hash, 0, hashsz * sizeof(int16));
	...
	while (dp < dend)
	{
		...
		if (pglz_find_match(hist_start, dp, dend, &match_len,
							&match_off, good_match, good_drop, mask, rolling_hash))
		{
			/*
